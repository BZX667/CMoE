...
"""
论文《Improving Alignment and Uniformity of Expert Representation with Contrastive Learning for Mixture of Experts model》在Deepseek MoE（2024年8约发布）模型上的应用。 通过在不同专家的hidden states之间增加正交约束， 从而缓解专家利用不均衡， 表征退化， 表征坍缩等问题
"""
    def EHC(self, experts):
        weights = []
        for expert in experts:
            weights.append(expert.down_proj.weight)
        W = torch.stack(weights)

        # U, S, Vh = torch.linalg.svd(W.to(torch.float32))
        # # 选择要保留的奇异值数量，这里假设保留 100 个奇异值用于降维
        # num_singular_values = 100
        # # 截取前 num_singular_values 个奇异值及其对应的左奇异向量和右奇异向量
        # U_reduced = U[:, :, :num_singular_values]
        # S_reduced = S[:, :num_singular_values]
        # # Vh_reduced = Vh[:, :num_singular_values, :]
        # # 重构降维后的矩阵
        # S_diag = torch.diag_embed(S_reduced)
        # W_svd = torch.matmul(U_reduced, S_diag)
        # # W_svd = torch.matmul(U_reduced, torch.matmul(S_diag, Vh_reduced))
        # # print(W_svd.shape)
        
        cl_loss = self.compute_cosine_similarity_matrix(W)
        return cl_loss

    def compute_cosine_similarity_matrix(self, matrix):
        num_matrices = matrix.size(0)
        norms = torch.norm(matrix.view(num_matrices, -1), dim=1, keepdim=True)
        normalized_matrices = matrix / norms.view(-1, 1, 1)
        diff_matrix = torch.einsum('abc,dbc->ad', normalized_matrices, normalized_matrices)
        diagonal_mask = torch.eye(num_matrices, dtype=torch.bool)
        diff_matrix[diagonal_mask] = 0
        diff_matrix = diff_matrix.to(torch.float32)
        target = torch.zeros_like(diff_matrix)
        # 计算平滑 L1 损失
        smooth_l1_loss_fn = nn.SmoothL1Loss(reduction='mean')
        smooth_l1_loss = smooth_l1_loss_fn(diff_matrix, target)
        # print(diff_matrix)
        # print(smooth_l1_loss)
        return smooth_l1_loss

    # def compute_cl_loss(self, experts):
    #     device = torch.device('cuda')
    #     weights = []
    #     for expert in experts:
    #         weights.append(expert.down_proj.weight.flatten())
    #     W = torch.stack(weights)
    #     W = W.to(torch.float32)
    #     fc_layer1 = nn.Linear(W.size()[-1], 256).to(device)
    #     W = fc_layer1(W)
    #
    #     # print(W.shape)
    #     norms = torch.norm(W, dim=1, keepdim=True)
    #     W = W / norms
    #     diff_matrix = torch.mm(W, W.t())
    #     eye_matrix = torch.eye(diff_matrix.shape[0]).to(device)
    #     diff_matrix = diff_matrix - eye_matrix * diff_matrix
    #     target = torch.zeros_like(diff_matrix)
    #     # 计算平滑 L1 损失
    #     smooth_l1_loss_fn = nn.SmoothL1Loss(reduction='mean')
    #     smooth_l1_loss = smooth_l1_loss_fn(diff_matrix, target)
    ...
    #     return smooth_l1_loss
    #
    # def contrastive_loss(self, score_matrix):
    #     seqlen = score_matrix.shape[1]
    #     # 提取对角线元素
    #     gold_score = torch.diagonal(score_matrix, dim1=1, dim2=2)  # bsz x seqlen
    #     gold_score = gold_score.unsqueeze(-1)
    #     # print(gold_score.shape)
    #     # assert gold_score.shape == torch.Size([bsz, seqlen, 1])
    #
    #     # 计算差值矩阵
    #     difference_matrix = gold_score - score_matrix
    #     # assert difference_matrix.shape == torch.Size([bsz, seqlen, seqlen])
    #
    #     # 计算损失矩阵
    #     loss_matrix = margin - difference_matrix
    #     loss_matrix = torch.relu(loss_matrix)
    #
    #     # 创建基础掩码
    #     base_mask = torch.ones((seqlen, seqlen)) - torch.eye(seqlen)
    #     loss_mask = base_mask.unsqueeze(0).expand(bsz, -1, -1)
    ...
    def forward(self, hidden_states):
        ...
        if self.training:
            ...
            y =  y.view(*orig_shape)
            # cl_loss = self.compute_cl_loss(self.experts)
            # EHC_loss = self.EHC(self.experts)
            y = AddAuxiliaryLoss.apply(y, aux_loss)
            # print(aux_loss)
            # y = AddAuxiliaryLoss.apply(y, EHC_loss)
        else:
