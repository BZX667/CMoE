...
tf.app.flags.DEFINE_string("model_dir", '/data/model_ckpt/', "model check point dir")
tf.app.flags.DEFINE_string("servable_model_dir", '/data/servable_model/',
...
tf.app.flags.DEFINE_integer("num_experts", 0, "Num of experts")
tf.app.flags.DEFINE_integer("num_tasks", 0, "Num of task")
...
def input_fn(filenames, batch_size=32, num_epochs=1, perform_shuffle=True):
    ...
    # dataset = dataset.padded_batch(batch_size, padded_shapes=({"feeds_ids": [None], "feeds_vals": [None], "title_ids": [None]}, [None]))
...
def custom_model_fn(mode, feat_ids, x_concat, loss, optimizer, expert_concat, learning_rate):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        if FLAGS.cl_loss:
            ...
            x_concat_map, affine1 = loss_function.projection_head_map(x_concat, head_mode)

            x_concat_noise = loss_function.add_gaussian_noise(x_concat, std=0.01)
            x_concat_drop = loss_function.random_dropout_embedding(x_concat_noise, keep_prob=0.95)
            # x_concat_cfm = loss_function.cfm_masking_F_F(x_concat, 0.1, inner_size)
            # x_concat_cfm, mask_ind = loss_function.cfm_masking_F_F_feat_mi(x_concat, feat_ids, 0.1, 84)
            # x_concat_cfm, mask_ind = loss_function.cfm_masking_F_F_feat(x_concat_cfm, feat_ids, 0.5)
            x_concat_cfm_map, affine2 = loss_function.projection_head_map(x_concat_drop, 1-head_mode)
            # affine1_print = tf.Print(affine1, [affine1], "affine1: %s" % affine1)
            # affine2_print = tf.Print(affine2, [affine2], "affine2: %s" % affine2)
            # masked_ind_print = tf.Print(masked_ind, [masked_ind], "masked_ind: %s" % masked_ind)
            # x_concat_print = tf.Print(x_concat_cfm, [x_concat_cfm], "x_concat_cfm: %s" % x_concat_cfm)
            # x_concat_cfm = loss_function.cfm_masking_F_L(x_concat_cfm, 0.05, y)
            # x_concat_noise_map = loss_function.projection_head_map(x_concat_noise, inner_size, mode)
            ...
            learning_rate_exp = tf.train.exponential_decay(0.006, global_step=tf.train.get_global_step(), decay_steps=3, decay_rate=0.96,
                                                       staircase=True)
            ...
            if (expert_loss & sample_loss):
                ...
                main_task_train_op = optimizer.minimize(loss, global_step=tf.train.get_global_step())
                train_op = tf.group(main_task_train_op, cl_train_op, cts_samples_train_op)
                hooks = [
                    tf.train.LoggingTensorHook(
                        {
                            "cl_loss": cl_loss_print,
                            "cts_sample_loss": cts_loss_diff_samples_print,
                            "step": step_print,
                ...
                cl_train_op = cl_optimizer.minimize(cl_loss, global_step=tf.train.get_global_step())
                train_op = tf.group(main_task_train_op, cl_train_op)
                ...
                cts_samples_train_op = cts_samples_optimizer.minimize(cts_loss_diff_samples,global_step=tf.train.get_global_step())
                train_op = tf.group(main_task_train_op, cts_samples_train_op)
                hooks = [
                    tf.train.LoggingTensorHook(
                        {
                            "cts_sample_loss": cts_loss_diff_samples_print,
                            ...
                            # "sample2": affine2_print,
                            # "lr": lr_print,
                            # "masked_ind": masked_ind_print,
                            # "x_concat_cfm": x_concat_print
                        },
                        every_n_iter=1
                    )
                ]
...
def model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope("Shared-Embedding-layer"):
        ...
        # x_concat = tf.concat([tf.reshape(common_embs, shape=[-1, common_dims])], axis=1)


    with tf.variable_scope("Experts_out"):
        expert_outs = []
        for i in range(num_experts):
            expert_network = DNN(expert_dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout, dnn_use_bn,
                                 seed=seed,
                                 name='expert_' + str(i))(x_concat)
    ...
    with tf.variable_scope("Mmoe_out"):
        ...
        for i in range(num_tasks):  # one mmoe layer: nums_tasks = num_gates
            ...
            gate_input = DNN(gate_dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout, dnn_use_bn, seed=seed,
                             name='gate_' + task_names[i])(x_concat)
    ...
    # tf.summary.scalar('cdr_loss', cdr_loss)
    ...
    train_op, hooks = custom_model_fn(mode, feat_ids, x_concat, loss, optimizer, expert_concat, learning_rate)
...
def main(_):
    # ------check Arguments------
    if FLAGS.dt_dir == "":
        FLAGS.dt_dir = (date.today() + timedelta(-7)).strftime('%Y-%m-%d')
    ...
    print('num_experts ', FLAGS.num_experts)
    print('num_tasks ', FLAGS.num_tasks)
