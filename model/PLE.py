...
def model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        if FLAGS.cl_loss:
            ...
            # x_concat_cfm = loss_function.random_dropout_embedding(x_concat_cfm, keep_prob=0.95)
            # x_concat_cfm = loss_function.cfm_masking_F_L(x_concat_cfm, 0.05, y)
            # x_concat_cfm = loss_function.add_gaussian_noise(x_concat_cfm, std=0.01)
            # x_concat_noise = loss_function.add_uniform_noise(x_concat, weight=0.001)
            # x_concat_cfm = loss_function.add_gaussian_noise(x_concat_cfm, std=0.01)
            ...
            if (expert_loss & sample_loss):
                ...
                cl_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate*1/3, beta1=0.9, beta2=0.999, epsilon=1e-8)
                cl_train_op = cl_optimizer.minimize(cl_loss, global_step=tf.train.get_global_step())

                cts_loss_diff_samples = 0.2 * loss_function.cts_loss_diff_samples(z_i=x_concat_map, z_j=x_concat_cfm_map, temp=0.01, batch_size=FLAGS.batch_size)
                cts_loss_diff_samples_print = tf.Print(cts_loss_diff_samples, [cts_loss_diff_samples], "cl_loss: %.16f")
                cts_samples_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate*1/6, beta1=0.9, beta2=0.999, epsilon=1e-8)
                ...
                cl_loss_print = tf.Print(cl_loss, [cl_loss], "cl_loss: %.16f")
                cl_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate*0.23, beta1=0.9, beta2=0.999, epsilon=1e-8)
                cl_train_op = cl_optimizer.minimize(cl_loss, global_step=tf.train.get_global_step())
